...
pub fn extract_frame(tools: &Tools, path: &Path, time: f64, out: &Path) -> Result<(), String> {
    ...
    cmd.args(["-hide_banner", "-y", "-loglevel", "error", "-ss", &format!("{:.4}", time.max(0.0)), "-i"])
        .arg(path)
...
pub fn detect_silence(tools: &Tools, path: &Path, in_: f64, out: f64, threshold_db: f64, min_duration: f64) -> Result<Vec<(f64, f64)>, String> {
    let len = (out - in_).max(0.0);
    let mut cmd = command(&tools.ffmpeg);
    cmd.args(["-hide_banner", "-nostats", "-ss", &format!("{in_:.4}"), "-t", &format!("{len:.4}"), "-i"])
        .arg(path)
        .args(["-vn", "-af", &format!("silencedetect=n={threshold_db}dB:d={}", min_duration.max(0.05)), "-f", "null", "-"]);
    let output = cmd.output().map_err(|e| format!("Cannot run ffmpeg: {e}"))?;
    ...
    Ok(ranges.into_iter().filter(|(a, b)| b - a >= min_duration * 0.5).collect())
}
...
fn envelope(tools: &Tools, path: &Path, start: f64, max_secs: f64) -> Result<Vec<f32>, String> {
    const SR: usize = 8000;
    let mut cmd = command(&tools.ffmpeg);
    cmd.args(["-hide_banner", "-loglevel", "error", "-ss", &format!("{start:.4}"), "-t", &format!("{max_secs:.3}"), "-i"])
        .arg(path)
...
pub fn sync_offset(tools: &Tools, a: &Path, in_a: f64, b: &Path, in_b: f64, max_lag: f64, max_secs: f64) -> Result<f64, String> {
    ...
    for lag in -max_lag_n..=max_lag_n {
        ...
        let (start, end) = if lag >= 0 { (0i64, (ea.len() as i64).min(eb.len() as i64 - lag)) } else { (-lag, (ea.len() as i64).min(eb.len() as i64 - lag)) };
...
pub fn transcribe(tools: &Tools, whisper: &Path, model: &Path, path: &Path, in_: f64, out: f64, language: &str, work: &Path) -> Result<String, String> {
    std::fs::create_dir_all(work).map_err(|e| e.to_string())?;
    let wav = work.join("speech.wav");
    let len = (out - in_).max(0.1);
    let mut cmd = command(&tools.ffmpeg);
    cmd.args(["-hide_banner", "-y", "-loglevel", "error", "-ss", &format!("{in_:.4}"), "-t", &format!("{len:.4}"), "-i"])
        .arg(path)
    ...
    w.arg("-m").arg(model).arg("-f").arg(&wav).arg("-osrt").arg("-of").arg(&prefix).arg("-nt");
    ...
    if !srt.exists() {
        return Err(format!(
            ...
            String::from_utf8_lossy(&output.stdout).chars().take(800).collect::<String>(),
...
mod tests {
    ...
    fn silence_regex_parses_ffmpeg_lines() {
        ...
        let c = re.captures("[silencedetect @ 0x1] silence_start: 1.234").unwrap();
        assert_eq!(&c[1], "1.234");
    }
}
