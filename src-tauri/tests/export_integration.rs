...
fn gen(ffmpeg: &Path, out: &Path, color: &str, secs: f64, tone: u32) {
    let st = Command::new(ffmpeg)
        .args([
            ...
            "lavfi",
            "-i",
            &format!("color=c={color}:s=320x180:r=30:d={secs}"),
            "-f",
            "lavfi",
            "-i",
            &format!("sine=frequency={tone}:d={secs}"),
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
...
fn exports_a_real_project() {
    let Some(t) = tools() else {
        eprintln!("skipping: set RVE_FFMPEG_DIR");
        return;
    };
    ...
    p.media.extend([
        media("red", &red, &red_info),
        media("blue", &blue, &blue_info),
    ]);
    ...
    b.effects.push(Effect {
        kind: "color".into(),
        params: HashMap::from([
            ("saturation".to_string(), serde_json::json!(1.2)),
            ("exposure".to_string(), serde_json::json!(0.2)),
            ("temperature".to_string(), serde_json::json!(7000)),
        ]),
        enabled: true,
    });
    b.reverse = true;
    ...
    let st = Command::new(&t.ffmpeg)
        .args([
            ...
            "lavfi",
            "-i",
            "color=green:s=160x90:d=1,format=rgba",
            "-vf",
            "pad=640:360:400:250:color=#00000000",
            "-frames:v",
            "1",
        ])
        .arg(&shape_png)
    ...
    p.tracks = vec![
        ...
        Track {
            ...
            kind: "video".into(),
            name: "V1".into(),
            clips: vec![clip("video", Some("red"), 0.0, 0.0, 2.0), b],
            ..Default::default()
        },
        Track {
            id: "a1".into(),
    ...
    assert!(
        (info.duration - expected).abs() < 0.15,
        "duration {}",
        info.duration
    );
    ...
    assert!(
        r > 200 && g_ < 60 && bl < 60,
        "expected red at 0.3s, got {:?}",
        (r, g_, bl)
    );
    let (r, g_, bl) = pixel(&t.ffmpeg, &out, 3.5, 320, 300);
    assert!(
        bl > 200 && r < 60 && g_ < 60,
        "expected blue at 3.5s, got {:?}",
        (r, g_, bl)
    );
    ...
    assert!(
        g_ > 100 && r < 60,
        "expected green shape at 1.0s, got {:?}",
        (r, g_)
    );
